...
def position_planer(pose, time_use):
    ...
    '''
    if(time_use < 10.0):
        pose.pose.position.z = (time_use/10.0)*5
    elif(time_use < 20.0 and (not tag_found)):
        pose.pose.position.x = ((time_use-10.0)/10.0)*15.0
    elif(time_use < 120.0 and pose.pose.position.z > 0.5):
...
if __name__ == "__main__":
    ...
    imudata_sub = rospy.Subscriber("mavros/imu/data", Imu, callback = imudata_cb)
    ...
    orig_pose_x = pose.pose.position.x = 0
    orig_pose_y = pose.pose.position.y = 0
    orig_pose_z = pose.pose.position.z = 0.5
    ...
    while(not rospy.is_shutdown()):
        if((not mission_finished) and current_state.mode != "OFFBOARD" and (rospy.Time.now() - last_req) > rospy.Duration(2.0)):
            ...
            last_req = rospy.Time.now()
            orig_pose_x = pose.pose.position.x
            orig_pose_y = pose.pose.position.y
            orig_pose_z = pose.pose.position.z
            ...
            if(not position_planer(pose, rospy.Time.now().to_sec() - last_req.to_sec())):
                if (rospy.Time.now().to_sec() - last_req.to_sec() > flight_time_len):
                    if(rospy.Time.now().to_sec() - last_req.to_sec() > flight_time_len+3.0):
